...
export default function BrainrotTracker() {
  ...
  const dataLoaded = useRef(false); // prevents writes until Firebase data is loaded
  ...
  const setWatchedPetsFb = (valOrFn) => {
    setWatchedPets((prev) => {
      const next = typeof valOrFn === "function" ? valOrFn(prev) : valOrFn;
      safeFbWrite("watchedPets", next);
      return next;
    });
  };
  const setAlertThresholdFb = (val) => {
    setAlertThreshold(val);
    safeFbWrite("alertThreshold", val);
  };
  ...
  const handleAddToBatch = () => {
    ...
    setBatchItems((prev) => {
      // Merge only if same pet+msType+msValue+mutation already in batch
      const dupIdx = prev.findIndex((b) => b.petName === item.petName && b.msType === item.msType && b.msValue === item.msValue && b.mutation === item.mutation);
      if (dupIdx >= 0) {
        const updated = [...prev];
